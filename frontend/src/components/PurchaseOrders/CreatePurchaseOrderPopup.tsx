import React, { useState, useEffect } from 'react';
import { createPortal } from 'react-dom';
import { X, ChevronDown, Plus, Upload, Trash2 } from 'lucide-react';
import api from '../../api';
import { PopupAnimation, useAnimationState } from '../../utils/AnimationUtils';
import { useNotification } from '../../components/Notifications';

interface Product {
  id: number;
  name?: string;
  product_name?: string;
  price?: string;
  base_price?: string;
  bdt_price?: number;
}

interface Client {
  id: number;
  company: string;
  contact: string;
}

interface SelectedProduct {
  productId: number;
  productName: string;
  quantity: number;
}

interface CreatePurchaseOrderPopupProps {
  isOpen: boolean;
  onClose: () => void;
  onCreate: (orderData: any) => void;
}

const CreatePurchaseOrderPopup: React.FC<CreatePurchaseOrderPopupProps> = ({
  isOpen,
  onClose,
  onCreate,
}) => {
  const { showError, showInfo } = useNotification();

  const [formData, setFormData] = useState({
    poNumber: '',
    status: 'Draft',
    client: '',
    clientId: 0,
    subscriptionType: '',
    recurringCount: 1,
    deliveryDate: '',
    subscriptionActive: false,
    poDetails: '',
  });

  const [selectedProducts, setSelectedProducts] = useState<SelectedProduct[]>([]);
  const [currentProductId, setCurrentProductId] = useState<number>(0);
  const [currentQuantity, setCurrentQuantity] = useState<number>(1);

  const [dropdownOpen, setDropdownOpen] = useState({
    status: false,
    client: false,
    product: false,
    subscriptionType: false,
  });

  const [loading, setLoading] = useState(false);
  const [poNumberLoading, setPoNumberLoading] = useState(false);
  const [clients, setClients] = useState<Client[]>([]);
  const [products, setProducts] = useState<Product[]>([]);
  const [attachments, setAttachments] = useState<File[]>([]);

  const statusOptions = ['Draft', 'Active', 'In Progress', 'Completed'];
  const subscriptionTypeOptions = [1, 2, 3, 6, 12]; // months

  useEffect(() => {
    if (isOpen) {
      fetchData();
      setSelectedProducts([]);
      setAttachments([]);
      setCurrentProductId(0);
      setCurrentQuantity(1);
      setFormData(prev => ({
        ...prev,
        subscriptionActive: false,
        subscriptionType: '',
        recurringCount: 1,
        deliveryDate: '',
        poDetails: '',
      }));
    }
  }, [isOpen]);

  const fetchData = async () => {
    setLoading(true);
    try {
      const [clientsRes, productsRes] = await Promise.all([
        api.get('/clients'),
        api.get('/products')
      ]);
      setClients(clientsRes.data || []);
      setProducts(productsRes.data || []);
      fetchAutoGeneratedPoNumber();
    } catch (error) {
      console.error('Error fetching data:', error);
      showError('Failed to load data');
    } finally {
      setLoading(false);
    }
  };

  const fetchAutoGeneratedPoNumber = async () => {
    setPoNumberLoading(true);
    try {
      const res = await api.get('/purchases/generate-po');
      if (res.data?.po_number) {
        setFormData(prev => ({ ...prev, poNumber: res.data.po_number }));
      }
    } catch (error) {
      const year = new Date().getFullYear();
      setFormData(prev => ({ ...prev, poNumber: `PO-${year}-0001` }));
    } finally {
      setPoNumberLoading(false);
    }
  };

  const toggleDropdown = (key: keyof typeof dropdownOpen) => {
    setDropdownOpen(prev => ({
      ...prev,
      [key]: !prev[key],
      ...(key !== 'status' && { status: false }),
      ...(key !== 'client' && { client: false }),
      ...(key !== 'product' && { product: false }),
      ...(key !== 'subscriptionType' && { subscriptionType: false }),
    }));
  };

  const selectClient = (client: Client) => {
    setFormData(prev => ({
      ...prev,
      client: `${client.company}`,
      clientId: client.id
    }));
    toggleDropdown('client');
  };

  const selectStatus = (status: string) => {
    setFormData(prev => ({ ...prev, status }));
    toggleDropdown('status');
  };

  const selectSubscriptionType = (type: number) => {
    setFormData(prev => ({ ...prev, subscriptionType: type.toString() }));
    toggleDropdown('subscriptionType');
  };

  const addProduct = () => {
    if (currentProductId === 0) return showInfo('Please select a product');
    if (currentQuantity < 1) return showInfo('Quantity must be at least 1');

    const product = products.find(p => p.id === currentProductId);
    if (!product) return;

    const existingIndex = selectedProducts.findIndex(p => p.productId === currentProductId);
    if (existingIndex !== -1) {
      setSelectedProducts(prev => {
        const updated = [...prev];
        updated[existingIndex].quantity += currentQuantity;
        return updated;
      });
    } else {
      setSelectedProducts(prev => [...prev, {
        productId: currentProductId,
        productName: product.product_name || product.name || 'Unknown',
        quantity: currentQuantity
      }]);
    }

    setCurrentProductId(0);
    setCurrentQuantity(1);
    toggleDropdown('product');
  };

  const removeProduct = (index: number) => {
    setSelectedProducts(prev => prev.filter((_, i) => i !== index));
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();

    if (!formData.clientId) return showInfo('Please select a client');
    if (selectedProducts.length === 0) return showInfo('Add at least one product');

    if (formData.subscriptionActive) {
      if (!formData.subscriptionType) return showInfo('Select subscription type');
      if (!formData.deliveryDate) return showInfo('Select delivery date');
      if (formData.recurringCount < 1) return showInfo('Recurring count must be ≥1');
    }

    const data = {
      po_number: formData.poNumber,
      status: formData.status,
      client_id: formData.clientId,
      products: selectedProducts.map(p => ({ productId: p.productId, quantity: p.quantity })),
      subscription_type: formData.subscriptionType ? formData.subscriptionType : null,
      recurring_count: formData.subscriptionType ? formData.recurringCount : null,
      delivery_date: formData.deliveryDate ? formData.deliveryDate : null,
      subscription_active: formData.subscriptionActive,
      po_details: formData.poDetails ? formData.poDetails : null,
      attachment: attachments[0] || null,
    };

    onCreate(data);
    onClose();
  };

  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files || []);
    const validFiles = files.filter(file => {
      if (file.size > 10 * 1024 * 1024) {
        showError(`${file.name} too large (max 10MB)`);
        return false;
      }
      return true;
    });
    setAttachments(validFiles.slice(0, 5));
  };

  const removeAttachment = (i: number) => {
    setAttachments(prev => prev.filter((_, idx) => idx !== i));
  };

  const formatFileSize = (bytes: number) => {
    if (bytes < 1024) return bytes + ' B';
    if (bytes < 1024 * 1024) return (bytes / 1024).toFixed(1) + ' KB';
    return (bytes / (1024 * 1024)).toFixed(1) + ' MB';
  };

  const { isVisible, isAnimating } = useAnimationState(isOpen);
  if (!isVisible) return null;

  return createPortal(
    <div
      className={`fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 px-4 transition-opacity duration-300 ${
        isAnimating ? 'opacity-100' : 'opacity-0'
      }`}
      onClick={onClose}
    >
      <PopupAnimation animationType="zoomIn" duration="0.25s">
        <div
          className="w-full max-w-2xl rounded-lg bg-white shadow-2xl max-h-[92vh] overflow-hidden flex flex-col"
          onClick={e => e.stopPropagation()}
        >
          {/* Compact Header */}
          <div className="flex items-center justify-between border-b border-gray-100 bg-gray-50/50 px-4 py-2">
            <h2 className="text-sm font-bold text-gray-800">Create Purchase Order</h2>
            <button onClick={onClose} className="rounded-md p-1 text-gray-400 hover:bg-gray-200 hover:text-gray-600">
              <X size={18} />
            </button>
          </div>

          <div className="overflow-y-auto flex-1">
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
              {/* Row 1: Basic Info - More compact grid */}
              <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
                <div className="md:col-span-3">
                  <label className="block text-[11px] font-semibold text-gray-600 mb-0.5">PO Number</label>
                  <input
                    type="text"
                    value={poNumberLoading ? '...' : formData.poNumber}
                    readOnly
                    className="w-full rounded border border-gray-200 bg-gray-50 px-2 py-1 text-xs"
                  />
                </div>
                <div className="md:col-span-3">
                  <label className="block text-[11px] font-semibold text-gray-600 mb-0.5">Status</label>
                  <div className="relative">
                    <button
                      type="button"
                      onClick={() => toggleDropdown('status')}
                      className="w-full flex justify-between items-center rounded border border-gray-200 px-2 py-1 text-xs bg-white"
                    >
                      {formData.status}
                      <ChevronDown size={14} />
                    </button>
                    {dropdownOpen.status && (
                      <div className="absolute z-10 mt-1 w-full rounded bg-white shadow-lg border text-xs">
                        {statusOptions.map(opt => (
                          <button key={opt} type="button" onClick={() => selectStatus(opt)} className="w-full px-2 py-1.5 text-left hover:bg-blue-50">
                            {opt}
                          </button>
                        ))}
                      </div>
                    )}
                  </div>
                </div>
                <div className="md:col-span-6">
                  <label className="block text-[11px] font-semibold text-gray-600 mb-0.5">Client</label>
                  <div className="relative">
                    <button
                      type="button"
                      onClick={() => toggleDropdown('client')}
                      className="w-full flex justify-between items-center rounded border border-gray-200 px-2 py-1 text-xs bg-white text-left"
                    >
                      <span className={formData.client ? 'truncate' : 'text-gray-400'}>
                        {formData.client || 'Select client'}
                      </span>
                      <ChevronDown size={14} />
                    </button>
                    {dropdownOpen.client && (
                      <div className="absolute z-10 mt-1 w-full rounded bg-white shadow-lg border max-h-40 overflow-y-auto text-xs">
                        {clients.map(client => (
                          <button key={client.id} type="button" onClick={() => selectClient(client)} className="w-full px-2 py-1.5 text-left hover:bg-blue-50 border-b last:border-0">
                            <div className="font-medium">{client.company}</div>
                          </button>
                        ))}
                      </div>
                    )}
                  </div>
                </div>
              </div>

              {/* Product Add Section - Tightened */}
              <div className="bg-gray-50/50 p-2 rounded-md border border-gray-100">
                <label className="block text-[11px] font-bold text-gray-700 mb-1.5">Add Products</label>
                <div className="grid grid-cols-1 md:grid-cols-12 gap-2">
                  <div className="md:col-span-7">
                    <div className="relative">
                      <button
                        type="button"
                        onClick={() => toggleDropdown('product')}
                        className="w-full flex justify-between items-center rounded border border-gray-200 px-2 py-1 text-xs bg-white text-left"
                      >
                        <span className="truncate">
                          {products.find(p => p.id === currentProductId)?.product_name || 'Select product'}
                        </span>
                        <ChevronDown size={14} />
                      </button>
                      {dropdownOpen.product && (
                        <div className="absolute z-10 mt-1 w-full rounded bg-white shadow-lg border max-h-40 overflow-y-auto text-xs">
                          {products.map(p => (
                            <button key={p.id} type="button" onClick={() => {
                              // Automatically add the selected product to the list
                              const existingIndex = selectedProducts.findIndex(sp => sp.productId === p.id);
                              if (existingIndex !== -1) {
                                // If product already exists, increase quantity
                                setSelectedProducts(prev => {
                                  const updated = [...prev];
                                  updated[existingIndex].quantity += 1;
                                  return updated;
                                });
                              } else {
                                // If new product, add it to the list
                                setSelectedProducts(prev => [...prev, {
                                  productId: p.id,
                                  productName: p.product_name || p.name || 'Unknown',
                                  quantity: 1
                                }]);
                              }
                              // Reset the current product selection
                              setCurrentProductId(0);
                              setCurrentQuantity(1);
                              toggleDropdown('product');
                            }} className="w-full px-2 py-1.5 text-left hover:bg-blue-50 border-b last:border-0">
                              <div>{p.product_name || p.name}</div>
                              <div className="text-[10px] text-gray-500">৳{p.bdt_price || p.price}</div>
                            </button>
                          ))}
                        </div>
                      )}
                    </div>
                  </div>
                  <div className="md:col-span-3">
                    <input
                      type="number"
                      min="1"
                      value={currentQuantity}
                      onChange={e => setCurrentQuantity(Math.max(1, parseInt(e.target.value) || 1))}
                      className="w-full rounded border border-gray-200 px-2 py-1 text-xs"
                      placeholder="Qty"
                    />
                  </div>
                  <div className="md:col-span-2">
                    <button type="button" onClick={addProduct} className="w-full py-1 bg-blue-600 text-white rounded text-xs font-medium flex items-center justify-center gap-1 hover:bg-blue-700">
                      <Plus size={14} /> Add
                    </button>
                  </div>
                </div>

                {selectedProducts.length > 0 && (
                  <div className="mt-2 bg-white rounded border border-gray-200 overflow-hidden">
                    <div className="max-h-32 overflow-y-auto divide-y divide-gray-50">
                      {selectedProducts.map((item, i) => (
                        <div key={i} className="flex justify-between items-center px-2 py-1.5 hover:bg-gray-50">
                          <div className="text-[11px] flex-1 truncate mr-2">
                            <span className="font-medium">{item.productName}</span>
                            <span className="text-gray-400 ml-2">(Qty: {item.quantity})</span>
                          </div>
                          <button type="button" onClick={() => removeProduct(i)} className="text-red-400 hover:text-red-600">
                            <Trash2 size={13} />
                          </button>
                        </div>
                      ))}
                    </div>
                  </div>
                )}
              </div>

              {/* Subscriptions & Delivery - side by side */}
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      id="sub"
                      checked={formData.subscriptionActive}
                      onChange={e => setFormData(prev => ({ ...prev, subscriptionActive: e.target.checked }))}
                      className="h-3.5 w-3.5 rounded border-gray-300"
                    />
                    <label htmlFor="sub" className="text-[11px] font-semibold text-gray-700">Subscription</label>
                  </div>
                  {formData.subscriptionActive && (
                    <div className="flex gap-2">
                      <div className="flex-1">
                        <button
                          type="button"
                          onClick={() => toggleDropdown('subscriptionType')}
                          className="w-full flex justify-between items-center rounded border border-gray-200 px-2 py-1 text-[11px] bg-white"
                        >
                          {formData.subscriptionType ? `${formData.subscriptionType} mo` : 'Period'}
                          <ChevronDown size={12} />
                        </button>
                        {dropdownOpen.subscriptionType && (
                          <div className="absolute z-10 mt-1 w-full rounded bg-white shadow-lg border max-h-40 overflow-y-auto text-[10px]">
                            {subscriptionTypeOptions.map(type => (
                              <button
                                key={type}
                                type="button"
                                onClick={() => selectSubscriptionType(type)}
                                className="w-full px-2 py-1.5 text-left hover:bg-blue-50 border-b last:border-0"
                              >
                                {type} Month{type > 1 ? 's' : ''}
                              </button>
                            ))}
                          </div>
                        )}
                      </div>
                      <input
                        type="number"
                        min="1"
                        value={formData.recurringCount}
                        onChange={e => setFormData(prev => ({ ...prev, recurringCount: Math.max(1, parseInt(e.target.value) || 1) }))}
                        className="w-16 rounded border border-gray-200 px-2 py-1 text-[11px]"
                      />
                    </div>
                  )}
                </div>
                <div>
                  <label className="block text-[11px] font-semibold text-gray-600 mb-0.5">Delivery Date</label>
                  <input
                    type="date"
                    value={formData.deliveryDate}
                    onChange={e => setFormData(prev => ({ ...prev, deliveryDate: e.target.value }))}
                    className="w-full rounded border border-gray-200 px-2 py-1 text-xs"
                  />
                </div>
              </div>

              {/* Details & Upload - side by side */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-semibold text-gray-600 mb-0.5">PO Details</label>
                  <textarea
                    rows={2}
                    value={formData.poDetails}
                    onChange={e => setFormData(prev => ({ ...prev, poDetails: e.target.value }))}
                    className="w-full rounded border border-gray-200 px-2 py-1 text-xs resize-none"
                    placeholder="Instructions..."
                  />
                </div>
                <div>
                  <label className="block text-[11px] font-semibold text-gray-600 mb-0.5">Attachment (Max 1)</label>
                  <label className="flex items-center justify-center gap-2 border border-dashed border-gray-200 rounded p-2 cursor-pointer hover:bg-gray-50">
                    <Upload size={14} className="text-blue-500" />
                    <span className="text-[10px] text-gray-500 truncate">
                      {attachments.length > 0 ? attachments[0].name : 'Click to upload'}
                    </span>
                    <input type="file" className="hidden" onChange={handleFileChange} />
                  </label>
                  {attachments.length > 0 && (
                    <button type="button" onClick={() => removeAttachment(0)} className="text-[10px] text-red-500 mt-1 flex items-center gap-1">
                      <Trash2 size={10} /> Remove
                    </button>
                  )}
                </div>
              </div>
            </form>
          </div>

          {/* Fixed Footer */}
          <div className="flex justify-end gap-2 p-3 border-t border-gray-100 bg-gray-50/50">
            <button onClick={onClose} className="px-4 py-1.5 text-xs font-medium text-gray-600 border border-gray-200 rounded hover:bg-white">
              Cancel
            </button>
            <button onClick={handleSubmit} className="px-5 py-1.5 text-xs font-medium text-white bg-blue-600 rounded hover:bg-blue-700">
              Create Order
            </button>
          </div>
        </div>
      </PopupAnimation>
    </div>,
    document.body
  );
};

export default CreatePurchaseOrderPopup;